...
type
  ...
  public
    ...
    // был произведен left mouse click на отчете - необходима детализация - открытие детализированного отчета
    procedure report_main_click(string_value:string);
    // получить из StringGrid и позиции в StringGrid наименование торговой точки для данной записи
    function get_points_name(stringgrid:TstringGrid;position_column,position_row:integer):string;
    // получить из StringGrid и позиции в StringGrid дату для данной записи
    ...
    // проверка на попадание интервала отображения в интервал отбора
    procedure check_display_interval;
    // установка режима отображения, равного одному месяцу, но не меньше чем дата начала
    procedure set_display_interval(with_check:boolean=true);

  end;

var
  fmDovidka_4_2: TfmDovidka_4_2;

implementation

{$R *.DFM}

procedure TfmDovidka_4_2.FormCreate(Sender: TObject);
begin
   load_data;
   fmDataModule.query_dovidka_1.sql.clear;
end;

procedure TfmDovidka_4_2.load_data;
begin
...
procedure TfmDovidka_4_2.button_showClick(Sender: TObject);
begin
...
procedure TfmDovidka_4_2.reportGetValue(const ParName: String;
...
begin
   ...
   then begin
        ParValue:=' период отображения данных - с '+self.edit_date_begin_display.text+' по '+self.edit_date_end_display.text;
        end;
...
function TfmDovidka_4_2.execute_query: boolean;
begin
...
//   and (self.edit_date_begin.date<=self.edit_date_end.date)
   then begin
        ...
        if self.checkbox_date_begin.Checked
        then begin
             fmDataModule.query_dovidka_1.SQL.add('WHERE MONEY.DATE_IN_OUT <='+chr(39)+self.edit_date_end.text+chr(39));
             end
        else begin
             fmDataModule.query_dovidka_1.SQL.add('WHERE MONEY.DATE_IN_OUT BETWEEN '+chr(39)+self.edit_date_begin.text+chr(39)+' AND '+chr(39)+self.edit_date_end.text+chr(39));
             end;
...
procedure TfmDovidka_4_2.query_to_grid(query: TIBQuery;
...
begin
   ...
   then begin
        ...
        if DATE_IN_OUT<>'невероятная дата ввода'
...
begin
   ...
   DATE_IN_OUT:='невероятная дата ввода';
   POINTS_NAME:='';
   row_counter:=0;
   expenses_1:=0;
   expenses_2:=0;
...
procedure TfmDovidka_4_2.reportManualBuild(Page: TfrPage);
var
   i:integer;
begin
...
procedure TfmDovidka_4_2.checkbox_date_beginClick(Sender: TObject);
begin
    if self.checkbox_date_begin.Checked
    then begin
...
procedure TfmDovidka_4_2.reportObjectClick(View: TfrView);
var
   temp_string:string;
begin
    try
...
procedure TfmDovidka_4_2.reportMouseOverObject(View: TfrView;
  var Cursor: TCursor);
var
   temp_string:string;
   position:integer;
...
procedure TfmDovidka_4_2.report_main_click(string_value: string);
var
...
begin
    try
        ...
        then begin
             ...
             self.report_sub.title:='Детальный отчет';
...
function TfmDovidka_4_2.get_date(stringgrid: TstringGrid;
                                 position_column,
...
function TfmDovidka_4_2.get_points_name(stringgrid: TstringGrid;
                                        position_column,
                                        position_row: integer): string;
...
function TfmDovidka_4_2.execute_sub_query(query,
                                          query_temp:TibQuery;
                                          string_date,
                                          string_point_name,
...
function TfmDovidka_4_2.get_expenses(position_column: integer): string;
var
   return_value:string;
   temp_value:integer;
begin
...
procedure TfmDovidka_4_2.set_display_interval(with_check:boolean=true);
begin
...
procedure TfmDovidka_4_2.check_display_interval;
begin
    // правильно ли выставлены даты выборки
    ...
    then begin
         ...
         // установлен ли флажек "с самого начала"
         if self.edit_date_begin.visible=false
         then begin
...
procedure TfmDovidka_4_2.edit_date_endChange(Sender: TObject);
begin
    self.set_display_interval(false);
end;

procedure TfmDovidka_4_2.edit_date_beginChange(Sender: TObject);
begin
     self.set_display_interval;
end;
